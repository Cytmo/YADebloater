...
def begin_to_get_functions():
    with open('temp1/pp.c.debloated.c','r') as f:
        code = f.read()
...
def compile(source,num=-1):
    ...
    cmd = 'gcc -w -o temp1/deb_{}.out {} > /dev/null 2>&1'.format(num,abspath)
    if num==-1:
        cmd = 'gcc -w -o temp1/deb.out {} > /dev/null 2>&1'.format('temp1/pp.c.debloated.c')
    p = os.system(cmd)
    return p

def execute(cmd):
    # print('Executing {}'.format(cmd))
    p = os.system('timeout 0.5 {}'.format(cmd))
    # try:
...
def run_test(src,num=0):
    file_temp1 = 'temp1_{}.c'.format(num)
    with open(file_temp1, 'w') as f:
        f.writelines(src)
    if compile(file_temp1,num)!=0:
        print('Compile Failed')
        return False
    else:
        pass
        # os.system('rm {}'.format(file_temp1))
    ...
    # cmds = []
    # current_work_dir = 'temp1'
    # BIN = 'temp1/deb_{}.out'.format(num)
    ...
    # # $BIN '@**?' 'b@t' < $INDIR/temp1-test/2179.inp.922.1 &>> $OUTDIR/o2
    # cmd2 = "{} '@**?' 'b@t' < {}/train/temp1-test/2179.inp.922.1 >> {}".format(BIN, current_work_dir, output_file)
    # # $BIN '?[^9-B]' 'a@n' < $INDIR/temp1-test/282.inp.126.2 &>> $OUTDIR/o3
    # cmd3 = "{} '?[^9-B]' 'a@n' < {}/train/temp1-test/282.inp.126.2 >> {}".format(BIN, current_work_dir, output_file)
    # # $BIN '-[^-z]' '&a@%' < $INDIR/temp1-test/768.inp.329.1 &>> $OUTDIR/o4
    # cmd4 = "{} '-[^-z]' '&a@%' < {}/train/temp1-test/768.inp.329.1 >> {}".format(BIN, current_work_dir, output_file)
    ...
    # # $BIN '%?[@@][^0-9][a-c]?-[^0-9]- [^9-B]$' 'NEW' < $INDIR/temp1-test/1129.inp.485.10 &>> $OUTDIR/o7
    # cmd7 = "{} '%?[@@][^0-9][a-c]?-[^0-9]- [^9-B]$' 'NEW' < {}/train/temp1-test/1129.inp.485.10 >> {}".format(BIN, current_work_dir, output_file)
    # # $BIN '-?@@*[_-z][^0-9]' '&' < $INDIR/temp1-test/821.inp.354.1 &>> $OUTDIR/o8
    # cmd8 = "{} '-?@@*[_-z][^0-9]' '&' < {}/train/temp1-test/821.inp.354.1 >> {}".format(BIN, current_work_dir, output_file)
    ...
    cmd1 = 'temp1/deb_{}.out -c < temp1/train/aaa.txt > output_{}'.format(num,num)  
    ret = execute(cmd1)
    cmd2 = 'diff temp1/standard_output output_{} > /dev/null 2>&1'.format(num)
    ret = execute(cmd2)
    os.system('rm output_{}'.format(num))
    os.system('rm temp1/deb_{}.out'.format(num))
...
def ddmin_function_level(code, test_func, function_list,num):
    ...
    for func in function_list:
        ...
        for i in range(func['start_line']+1, func['end_line'] , granularity):
            print('removing function {} ...iteraion {}...granularity is {}'.format(func['name'],cnt,granularity))
            cnt+=1
            ...
            if(i+granularity > func['end_line']):
                for k in range(i-1,max_length):
                    try :
                        skip_indicator += candidate[k]
                        candidate[k] = '\n'
                    except IndexError:
                        print('IndexError k = {}'.format(k))
                ...
                for k in range(i-1,i+granularity-1):
                    ...
                    print('Skipped')
            # Call the test function with the reduced code
            if granularity ==1 and skip_indicator.strip().replace('}','').replace('{','').replace(';','') == '':
                print('Skipped')                    
                continue
    ...
    print('Removing function {}...'.format(func['name']))
    print('Removing function {}...'.format(func['name']))
    with open('reduced_code.c','w') as f:
        f.writelines(code)
...
if(True):
    ...
    print (other_lines)
    print('Reducing global variables and other codes...')
    reduced_code = ddmin_execute(code_lines, run_test, other_lines)

    with open('temp1/pp.c.debloated.c','w+') as f:
...
# print(function_list)
function_list = begin_to_get_functions()
reduced_code= ddmin_function_level(code_lines,run_test,function_list,1)
