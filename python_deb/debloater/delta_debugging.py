...
def ddmin(code, test_func, line_list):
    ...
    while len(code) > 0 and granularity >= 1:
        ...
        while i < len(candidate)  and len(line_list) > 0:
            removed_code = []
            if not i in line_list:
                i += 1
                continue
            ...
            line_list_str = ''.join(str(line_list_reduced))
            cache_key = hashlib.md5(line_list_str.encode('utf-8')).hexdigest()
            if(not check_brackets_balance(str(candidate))):
...
def tree_based_hdd_for_functions(code, test_func, function_list, num):
    ...
    for func in tqdm(function_list, position=1,desc="Reducing functions ..."):
        ...
        line_tree_level_1 = line_tree.get_nodes_at_level(1)
        ...
        if function_length <= 1 or tree_level==1:
        ...
        for i in range(tree_level):
            ...
            while start_index < number_of_nodes:
                ...
                for node in nodes_to_remove:
                    start_index += 1
                    lines_in_node = list(range(int(node.data[0])+1, int(node.data[1])))
                    if not node.children:
                        
                        # lines_in_node_to_remove = []
                        # for k in lines_in_node:
                        #     if not if_remove_line(code, k):
                        #         lines_in_node_to_remove.append(k)
                        # for k in lines_in_node_to_remove:
                        #     lines_in_node.remove(k)
                        if len(lines_in_node) > 0:
                            logger.debug("Reducing node with ddmin: %s" % node)
                    ...
                    if len(lines_in_node) > 0:
                        need_test = True
                        for code_idx in range(int(node.data[0])+1, int(node.data[1])):
        ...
        lines_outside_of_blocks.extend(range(func["start_line"]+1,func["end_line"]))
        logger.debug("line tree level 1: %s" % line_tree_level_1)
        for node in line_tree_level_1:
            logger.debug("Removing lines in node %s" % node)
            for i in range(int(node.data[0]), int(node.data[1])+1):
                if i in lines_outside_of_blocks:
                    lines_outside_of_blocks.remove(i)
            logger.debug("Lines outside of blocks: %s" % lines_outside_of_blocks)
        if len(lines_outside_of_blocks) > 0:
            logger.debug("Reducing lines outside of blocks: %s" % lines_outside_of_blocks)
