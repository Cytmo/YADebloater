...
void argmatch_valid(const char *const *arglist, const char *vallist, size_t valsize)
{
  ...
  int tmp___2;
  {
    ...
    fputs_unlocked((const char *) tmp, stderr);
    i = (size_t) 0;
    while (1)
    {
      if (! (* (arglist + i)))
...
void version_etc_arn(FILE *stream, const char *command_name, const char *package, const char *version, const char *const *authors, size_t n_authors)
{
  ...
  char *tmp___10;
  {
    ...
    fputs_unlocked((const char *) tmp___0, stream);
    if (n_authors == 0UL)
    {
      goto case_0;
    }
    ...
    fprintf(stream, (const char *) tmp___1, * (authors + 0));
    goto switch_break;
    case_2:
    tmp___2 = gettext("Written by %s and %s.\n");

    fprintf(stream, (const char *) tmp___2, * (authors + 0), * (authors + 1));
    goto switch_break;
    case_3:
    tmp___3 = gettext("Written by %s, %s, and %s.\n");

    fprintf(stream, (const char *) tmp___3, * (authors + 0), * (authors + 1), * (authors + 2));
    goto switch_break;
    case_4:
    tmp___4 = gettext("Written by %s, %s, %s,\nand %s.\n");

    fprintf(stream, (const char *) tmp___4, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3));
    goto switch_break;
    case_5:
    tmp___5 = gettext("Written by %s, %s, %s,\n%s, and %s.\n");

    fprintf(stream, (const char *) tmp___5, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4));
    goto switch_break;
    case_6:
    tmp___6 = gettext("Written by %s, %s, %s,\n%s, %s, and %s.\n");

    fprintf(stream, (const char *) tmp___6, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4), * (authors + 5));
    goto switch_break;
    case_7:
    tmp___7 = gettext("Written by %s, %s, %s,\n%s, %s, %s, and %s.\n");

    fprintf(stream, (const char *) tmp___7, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4), * (authors + 5), * (authors + 6));
    goto switch_break;
    case_8:
    tmp___8 = gettext("Written by %s, %s, %s,\n%s, %s, %s, %s,\nand %s.\n");

    fprintf(stream, (const char *) tmp___8, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4), * (authors + 5), * (authors + 6), * (authors + 7));
    goto switch_break;
    case_9:
    tmp___9 = gettext("Written by %s, %s, %s,\n%s, %s, %s, %s,\n%s, and %s.\n");

    fprintf(stream, (const char *) tmp___9, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4), * (authors + 5), * (authors + 6), * (authors + 7), * (authors + 8));
    goto switch_break;
    switch_default:
    tmp___10 = gettext("Written by %s, %s, %s,\n%s, %s, %s, %s,\n%s, %s, and others.\n");

    fprintf(stream, (const char *) tmp___10, * (authors + 0), * (authors + 1), * (authors + 2), * (authors + 3), * (authors + 4), * (authors + 5), * (authors + 6), * (authors + 7), * (authors + 8));
    goto switch_break;
    switch_break:
    ;
...
int settime(const struct timespec *ts)
{
  ...
  int tmp___1;
  {
    ...
    tmp___1 = settimeofday((const struct timeval *) (& tv), (const struct timezone *) 0);
    return tmp___1;
  }
}
...
static int posix_time_parse(struct tm *tm, const char *s, unsigned int syntax_bits)
{
  ...
  int seconds;
  {
    dot = (const char *) ((void *) 0);
    tmp = strlen(s);
    s_len = tmp;
    if (syntax_bits & 8U)
    {
      dot = (const char *) strchr(s, '.');
      if (dot)
      {
    ...
    while (1)
    {
      ...
      pair[i] = ((10 * ((int) (((const int) (* (s + (2UL * i)))) - 48))) + ((int) (* (s + ((2UL * i) + 1UL))))) - 48;
      i++;
    }
    ...
    if (syntax_bits & 1U)
    {
      tmp___1 = year(tm, (const int *) p, len - 4UL, syntax_bits);
      if (tmp___1)
      {
        return 1;
      }
    ...
    if (syntax_bits & 2U)
    {
      tmp___6 = year(tm, (const int *) p, len, syntax_bits);
      if (tmp___6)
      {
        return 1;
      }
    ...
    else
    {
      ...
      seconds = ((10 * ((int) (((const int) (* (dot + 0))) - 48))) + ((int) (* (dot + 1)))) - 48;
      tm->tm_sec = seconds;
    }
...
_Bool posixtime(time_t *p, const char *s, unsigned int syntax_bits)
{
  ...
  _Bool tmp___1;
  {
    ...
    if (t != (- 1L))
    {
      tm = (const struct tm *) (& tm1);
    }
    else
    {
      tm = (const struct tm *) localtime((const time_t *) (& t));
      if (! tm)
      {
        return (_Bool) 0;
      }
    ...
    if ((((((tm0.tm_year ^ ((int) tm->tm_year)) | (tm0.tm_mon ^ ((int) tm->tm_mon))) | (tm0.tm_mday ^ ((int) tm->tm_mday))) | (tm0.tm_hour ^ ((int) tm->tm_hour))) | (tm0.tm_min ^ ((int) tm->tm_min))) | (tm0.tm_sec ^ ((int) tm->tm_sec)))
    {
      ...
      b = tmp___0;
      strcpy(b - 2, "59");
      tmp___1 = posixtime(& dummy, (const char *) buf___1, syntax_bits);
      if (! tmp___1)
      {
        return (_Bool) 0;
      }
  ...
  relative_time __constr_expr_39;
  {
    ...
    * yyssp = (yytype_int16) yystate;
    ...
    if (((unsigned long) ((yyss + yystacksize) - 1)) <= ((unsigned long) yyssp))
    {
      ...
      while (1)
      {
        __builtin_memcpy((void *) (& yyptr->yyss_alloc), (const void *) yyss, yysize * (sizeof(* yyss)));
      ...
      while (1)
      {
        __builtin_memcpy((void *) (& yyptr->yyvs_alloc), (const void *) yyvs, yysize * (sizeof(* yyvs)));
    ...
    * yyvsp = yyval;
    ...
    {
      yynerrs++;
      yyerror((const parser_control *) pc, "syntax error");
    }
...
static _Bool mktime_ok(const struct tm *tm0, const struct tm *tm1, time_t t)
{
  if (t == (- 1L))
  {
    tm1 = (const struct tm *) localtime((const time_t *) (& t));
    if (! tm1)
    {
      return (_Bool) 0;
    }
...
static char *get_tz(char *tzbuf)
{
  ...
  void *tmp___2;
  {
    ...
    if (tz)
    {
      tmp___0 = strlen((const char *) tz);
      tzsize = tmp___0 + 1UL;
      if (tzsize <= 100UL)
      {
        tmp___1 = memcpy((void *) tzbuf, (const void *) tz, tzsize);
        tz = (char *) tmp___1;
      }
      else
      {
        tmp___2 = xmemdup((const void *) tz, tzsize);
        tz = (char *) tmp___2;
      }
...
_Bool parse_datetime(struct timespec *result, const char *p, const struct timespec *now)
{
  ...
  int tmp___16;
  {
    ...
    else
    {
      ...
      if (! tmp___12)
      {
        ...
        else
        {
          ...
          sprintf(tz1buf___0, "XXX%s%ld:%02d", "-" + (time_zone < 0L), abs_time_zone_hour, abs_time_zone_min);
          tmp___10 = setenv("TZ", (const char *) tz1buf___0, 1);
          if (tmp___10 != 0)
          {
            goto fail;
          }
          ...
          tm = tm0;
          Start = mktime(& tm);
          tmp___11 = mktime_ok((const struct tm *) (& tm0), (const struct tm *) (& tm), Start);
          if (! tmp___11)
          {
            goto fail;
          }
    ...
    if (tz_was_altered)
    {
      if (tz0)
      {
        tmp___14 = setenv("TZ", (const char *) tz0, 1);
        tmp___16 = tmp___14;
      }
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            while (1)
            {
              ...
              strcpy(((res_ptr + res_size) - (l2 + 1UL)) - (l1 + 1UL), (const char *) buf1);
              strcpy((res_ptr + res_size) - (l2 + 1UL), (const char *) buf2);
              __Cont:
              ;
    ...
    while (1)
    {
      ...
      * u = (char) '\000';
        ...
        while (1)
        {
          ...
          if (p)
          {
            ...
            while (1)
            {
              if (to_lowcase)
              {
                fwrite_lowcase(p, (const char *) (ubuf + 1), _n___2);
              }
              else
              {
                if (to_uppcase)
      ...
      while (1)
      {
        ...
        if (p)
        {
          ...
          while (1)
          {
            if (to_lowcase)
            {
              fwrite_lowcase(p, (const char *) bufp, _n___7);
            }
            else
            {
              if (to_uppcase)
              {
                fwrite_uppcase(p, (const char *) bufp, _n___7);
              }
              else
              {
      ...
      if (days < 0)
      {
        ...
        days = iso_week_days((int) (tp->tm_yday + ((const int) (365 + tmp___32))), (int) tp->tm_wday);
      }
      else
      {
        if ((year___1 % 4) == 0)
      ...
      negative_number = (_Bool) (tp->tm_year < ((const int) ((- 1900) - year_adjust)));
        ...
        if (p)
        {
          ...
          while (1)
          {
            ...
            else
            {
              ...
              {
                fwrite((const void *) (f + (1 - flen)), _n___11, (size_t) 1, p);
              }
...
__inline static void emit_mandatory_arg_note(void)
{
  char *tmp;
  {
    ...
    fputs_unlocked((const char *) tmp, stdout);
    return;
  }
}
...
__inline static void emit_ancillary_info(void)
{
  ...
  char *tmp___8;
  {
    ...
    fputs_unlocked((const char *) tmp___2, stdout);
...
__inline static void emit_try_help(void)
{
  char *tmp;
  {
    ...
    fprintf(stderr, (const char *) tmp, program_name);
    return;
  }
}
...
static _Bool batch_convert(const char *input_filename, const char *format)
{
  ...
  int tmp___9;
  {
    ...
    if (tmp___1 == 0)
    {
      input_filename = (const char *) gettext("standard input");
      in_stream = stdin;
    }
    else
    {
    ...
    while (1)
    {
      ...
      tmp___6 = parse_datetime(& when, (const char *) line, (const struct timespec *) ((void *) 0));
      if (tmp___6)
      {
      ...
      else
      {
        ...
        tmp___3 = quote((const char *) line);
        tmp___4 = gettext("invalid date %s");
        error(0, 0, (const char *) tmp___4, tmp___3);
        ok = (_Bool) 0;
      }
...
static const char iso_8601_format[5][32] = {{(const char) '%', (const char) 'Y', (const char) '-', (const char) '%', (const char) 'm', (const char) '-', (const char) '%', (const char) 'd', (const char) '\000'}, {(const char) '%', (const char) 'Y', (const char) '-', (const char) '%', (const char) 'm', (const char) '-', (const char) '%', (const char) 'd', (const char) 'T', (const char) '%', (const char) 'H', (const char) ':', (const char) '%', (const char) 'M', (const char) ':', (const char) '%', (const char) 'S', (const char) '%', (const char) 'z', (const char) '\000'}, {(const char) '%', (const char) 'Y', (const char) '-', (const char) '%', (const char) 'm', (const char) '-', (const char) '%', (const char) 'd', (const char) 'T', (const char) '%', (const char) 'H', (const char) ':', (const char) '%', (const char) 'M', (const char) ':', (const char) '%', (const char) 'S', (const char) ',', (const char) '%', (const char) 'N', (const char) '%', (const char) 'z', (const char) '\000'}, {(const char) '%', (const char) 'Y', (const char) '-', (const char) '%', (const char) 'm', (const char) '-', (const char) '%', (const char) 'd', (const char) 'T', (const char) '%', (const char) 'H', (const char) '%', (const char) 'z', (const char) '\000'}, {(const char) '%', (const char) 'Y', (const char) '-', (const char) '%', (const char) 'm', (const char) '-', (const char) '%', (const char) 'd', (const char) 'T', (const char) '%', (const char) 'H', (const char) ':', (const char) '%', (const char) 'M', (const char) '%', (const char) 'z', (const char) '\000'}};
int main(int argc, char **argv)
{
  int optc;
  const char *datestr;
  ...
  int tmp___24;
  {
    ...
    while (1)
    {
      ...
      if (optarg)
      {
        tmp___0 = __xargmatch_internal("--iso-8601", (const char *) optarg, time_spec_string, (const char *) time_spec, sizeof(time_spec[0]), argmatch_die);
    ...
    if (optind < argc)
    {
      if ((optind + 1) < argc)
      {
        tmp___10 = quote((const char *) (* (argv + (optind + 1))));
        tmp___11 = gettext("extra operand %s");
        error(0, 0, (const char *) tmp___11, tmp___10);
        usage(1);
      }
      ...
      else
      {
        if (set_date)
        {
          tmp___14 = quote((const char *) (* (argv + optind)));
          tmp___15 = gettext("the argument %s lacks a leading \'+\';\nwhen using an option to specify date(s), any non-option\nargument must be a format string beginning with \'+\'");
          error(0, 0, (const char *) tmp___15, tmp___14);
          usage(1);
        }
        else
        {
    ...
    {
      ok = batch_convert((const char *) batch_file, format);
    }
    else
    {
      valid_date = (_Bool) 1;
      ...
      if (! option_specified_date)
      {
        if (! set_date)
        {
          ...
          {
            set_date = (_Bool) 1;
            datestr = (const char *) (* (argv + optind));
      ...
      else
      {
        ...
        {
          tmp___17 = stat((const char *) reference, & refstats);
          if (tmp___17 != 0)
          {
          ...
          when = get_stat_mtime((const struct stat *) (& refstats));
        }
        else
        {
          if (set_datestr)
      ...
      if (set_date)
      {
        tmp___22 = settime((const struct timespec *) (& when));
        if (tmp___22 != 0)
        {
          tmp___20 = gettext("cannot set date");
          tmp___21 = __errno_location();
          error(0, * tmp___21, (const char *) tmp___20);
          ok = (_Bool) 0;
        }
...
static _Bool show_date(const char *format, struct timespec when)
{
  ...
  char *tmp___0;
  {
    ...
    if (! tm)
    {
      ...
      error(0, 0, (const char *) tmp___0, tmp);
      return (_Bool) 0;
    }
